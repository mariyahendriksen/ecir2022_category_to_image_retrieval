...
def main(args: argparse.Namespace) -> None:
    """
    Main function to perform retrieval evaluation.
    ...
    if is_zero_shot:
        ...
        print(f'Non-zero-shot setup, loading pre-trained model from {config["DATASET"][dataset]["model_path"]}...')
    ...
    image_dict_path = os.path.join(config['DATASET'][dataset]['root'], config['DATASET'][dataset]['image_dict'])

    if add_prefix:
        text_dict_file = config['DATASET'][dataset]['text_dict_prefix']
    else:
        text_dict_file = config['DATASET'][dataset]['text_dict']
    text_dict_path = os.path.join(config['DATASET'][dataset]['root'], text_dict_file)

    image_dict = load_pkl(image_dict_path)
    text_dict = load_pkl(text_dict_path)

    print('Loading the dataset...')
    df_test_path = os.path.join(config['DATASET'][dataset]['root'], config['DATASET'][dataset]['df_test'])
    ...
    text_query_embs = dict_to_list(
        indices=ids, dict=text_dict, trained_model=model, content_type='text'
    )
    ...
    t2i_query_ids, t2i_top_k_results, t2i_recalls_at_1, t2i_recalls_at_5, t2i_recalls_at_10, t2i_recalls_at_50, t2i_recalls_at_100 = iterate_over_results_to_compute_recalls(
        ids=ids, results=text_to_image_results, k_list=[1, 5, 10, 50, 100])

    t2i_results = pd.DataFrame(data={
    ...
    image_query_embs = dict_to_list(
        indices=ids, dict=image_dict, trained_model=model, content_type='image'
    )
    ...
    assert len(image_query_embs) == len(text_tensor)
    ...
    i2t_query_ids, i2t_top_k_results, i2t_recalls_at_1, i2t_recalls_at_5, i2t_recalls_at_10, i2t_recalls_at_50, i2t_recalls_at_100 = iterate_over_results_to_compute_recalls(
        ids=ids, results=image_to_text_results, k_list=[1, 5, 10, 50, 100])

    i2t_results = pd.DataFrame(data={
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Script to perform text-to-image and image-to-text retrieval evaluation."
    )
    parser.add_argument(
        '--config_file',
        type=str,
        default='CLIP/conf/train_conf.yaml',
        help='Configuration file'
    )
    parser.add_argument(
        '--ds_config_file',
        type=str,
        default='CLIP/conf/data_conf.yaml',
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help='Use CLIP in zero-shot way?'
    )
